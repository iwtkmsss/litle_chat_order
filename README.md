...
- підготовка записів `email_notifications` для майбутнього інформування замовника email-повідомленнями;
- окремі сценарії опитувальних листів: Додаток 4 для тепловикористальної установки та Додаток 5 для теплогенеруючої / когенераційної установки;
...
- `/` — публічна сторінка приєднання, однакова для всіх відвідувачів.
- `/status` — перевірка стану заяви.
- `/documents` — зразки документів.
- `/account` — особистий кабінет; вміст визначається cookie-сесією користувача.
  На цій сторінці замовник може увійти, а гість може подати нову заяву без негайного створення повного кабінету.
- `/application-access/:token` — тимчасовий кабінет pending-заяви до перевірки оператором.
